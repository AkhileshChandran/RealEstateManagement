...
def user_login(request):
    if request.method == "POST":
        ...
        if RegistrationDB.objects.filter(Email=un, Password=pwd).exists():
            ...
            data=messages.success(request, "Logged in Successfully..!!")
...
def search(request):
    if request.method=="POST":
            ...
            data=PropertyDB.objects.filter(Price=price,Type=p_type,Location=location)
            print(data)
            print(price,p_type,location)
...
def search(request):        
    if request.method == 'GET':
        property_list = request.GET.get('search') 
        if property_list:
            status = PropertyDB.objects.filter(Type=property_list)
        else:
            status = PropertyDB.objects.all()  # If no search query, return all properties
        return render(request, "serch.html", {"properties": status})
    else:
